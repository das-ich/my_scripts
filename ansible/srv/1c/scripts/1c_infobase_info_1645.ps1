...
function return_json($data_array=@(), $iserror=0)
{
	...
	if ($iserror -eq 1)
	{
		$line = $line + "`"ERRORMSG_1645`" : `""+$data_array[0]+"`"`n" 
	} else {
		$line = $line + "`"SESSIONS_1645`" : `""+$data_array[0]+"`",`n"
		$line = $line + "`"LOCK_BY_DBMS_1645`" : `""+$data_array[1]+"`",`n"
		$line = $line + "`"LOCK_BY_LS_1645`" : `""+$data_array[2]+"`",`n"
		$line = $line + "`"SESSIONS_DESIGNER_1645`" : `""+$data_array[3]+"`",`n"
		$line = $line + "`"SESSIONS_BACKGROUND_JOB_1645`" : `""+$data_array[4]+"`",`n"
		$line = $line + "`"SESSIONS_THIN_CLIENT_1645`" : `""+$data_array[5]+"`",`n"
		$line = $line + "`"SESSIONS_THICK_CLIENT_1645`" : `""+$data_array[6]+"`",`n"
		$line = $line + "`"SESSIONS_COM_CONNECTION_1645`" : `""+$data_array[7]+"`",`n"
		$line = $line + "`"SESSIONS_WEB_SERVICE_1645`" : `""+$data_array[8]+"`",`n"
		$line = $line + "`"SESSIONS_COM_CONSOLE_1645`" : `""+$data_array[9]+"`",`n"
		$line = $line + "`"SESSIONS_UNKNOWN_1645`" : `""+$data_array[10]+"`",`n"
		$line = $line + "`"MAX_DURATION_1645`" : `""+$data_array[11]+"`",`n"
		$line = $line + "`"MAX_DURATION_DBMS_1645`" : `""+$data_array[12]+"`",`n"
		$line = $line + "`"ERRORMSG_1645`" : `"`"`n" 
	}
