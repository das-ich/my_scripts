...
foreach ($cluster_description in $data_array){
	...
	{
		$infobase_count = 1
		$line = $line + "  { `"{#CLUSTERNAME_1645}`" : `"" + $cluster_name + "`", `"{#CLUSTERID_1645}`" : `"" + $cluster_id + "`", `"{#INFOBASENAME_1645}`" : `"" + $(get_value_from_text_line $infobase_description) + "`", `"{#INFOBASEID_1645}`" : `"" + $(get_key_from_text_line $infobase_description) + "`" },`n"
	}
