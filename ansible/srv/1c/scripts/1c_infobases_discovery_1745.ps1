...
foreach ($cluster_description in $data_array){
	...
	{
		$infobase_count = 1
		$line = $line + "  { `"{#CLUSTERNAME_1745}`" : `"" + $cluster_name + "`", `"{#CLUSTERID_1745}`" : `"" + $cluster_id + "`", `"{#INFOBASENAME_1745}`" : `"" + $(get_value_from_text_line $infobase_description) + "`", `"{#INFOBASEID_1745}`" : `"" + $(get_key_from_text_line $infobase_description) + "`" },`n"
	}
